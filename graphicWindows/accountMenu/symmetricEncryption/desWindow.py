...
def generate_des_window(main_screen):
    ...
    des_window = Toplevel(main_screen)

    des_window.geometry("700x500+380+180")
    des_window.title("DES protocol")
    des_window.resizable(False,False)

    des_window = ttk.Notebook(des_window)

    tab1 = Frame(des_window)
    tab2 = Frame(des_window)

    des_window.add(tab1, text ='Encrypt')
    des_window.add(tab2, text ='Decrypt')
    des_window.pack(expand = 5, fill ="both")
    ...
    Label(tab1,font=("Calibri", 8),fg="grey", text="files will be located under encryption/symmetricEncryption/des").pack(pady=(0,10))
...
def encrypt_text():
    ...
    if message_data and key_data and file_data:
        try:
            ...
            file_out = open("encryption/symmetricEncryption/des/"+file_data+".enc", "wb")
